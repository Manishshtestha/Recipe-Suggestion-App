...
const RegistrationSchema = z.object({
    ...
    password: z.string().min(8, { message: "Access Key (Password) must be at least 8 characters long." }),
});
...
const RegisterPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!validationResult.success) {
            ...
            setValidationError(firstError || "Invalid input. Please check the fields.");
            setIsLoading(false);
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {(validationError || apiError) && (
                        <p className="mt-4 text-center text-sm text-red-400 bg-red-900 bg-opacity-50 border border-red-500 p-2 rounded-none">
                            <span className='font-bold'>Error:</span> {validationError || apiError}
                        </p>
                    )}
                    {successMessage && (
