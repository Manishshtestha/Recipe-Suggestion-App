...
export default async function UniqueRecipe({ params }: RecipePageProps) {
  await dbConnect();
  // Make sure your RecipeModel and lean() object structure matches what's accessed below
      ...
      <div className="recipe-container bg-black bg-opacity-70 backdrop-blur-sm border-2 border-neutral-700 p-6 md:p-8 max-w-5xl mx-auto rounded-none shadow-lg shadow-cyan-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          <section className="mb-6">
            <h2 className="text-xl md:text-2xl font-semibold mb-3 text-pink-400 uppercase tracking-wider">// Components_List</h2>
            {recipe.ingredients && recipe.ingredients.length > 0 ? (
              <ul className="list-none flex flex-wrap gap-2">
                {recipe.ingredients.map((ingredient: string, index: number) => (
                  <li key={index} className="bg-neutral-800 border border-neutral-700 text-cyan-300 rounded-none px-3 py-1 text-sm hover:bg-neutral-700 hover:border-cyan-400 transition-all duration-150">
                    {toSentenceCase(ingredient)}
                  </li>
                ))}
              </ul>
            ) : <p className="text-neutral-500 italic">No component data available.</p>}
          </section>

          <section className="mb-6">
            <h2 className="text-xl md:text-2xl font-semibold mb-3 text-pink-400 uppercase tracking-wider">// Nutritional_Profile</h2>
            ...
            <ul className="list-none space-y-1">
              {recipe.nutrition.map((nutrient: string, index: number) => (
                <li key={index} className="text-green-400 text-sm">
                  <span className="text-pink-500 mr-1">{`>`}</span>{nutrient}
                </li>
              ))}
            </ul>
            ) : <p className="text-neutral-500 italic">No nutritional data logged.</p>}
          </section>
          
          {recipe.dietaryRestrictions && recipe.dietaryRestrictions.length > 0 && (
            <section className="mb-6 md:col-span-2">
              <h2 className="text-xl md:text-2xl font-semibold mb-3 text-pink-400 uppercase tracking-wider">// Dietary_Flags</h2>
              <ul className="list-none flex flex-wrap gap-2">
                {recipe.dietaryRestrictions.map((restriction: string, index: number) => (
                  <li key={index} className="bg-yellow-600 bg-opacity-30 border border-yellow-500 text-yellow-300 rounded-none px-3 py-1 text-sm">
                    {toSentenceCase(restriction)}
                  </li>
                ))}
              </ul>
            </section>
        ...
        <section className="my-8 border-t-2 border-neutral-700 pt-6">
          <h2 className="text-xl md:text-2xl font-semibold mb-4 text-pink-400 uppercase tracking-wider">// System_Parameters</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
            <DetailItem label="Cuisine Type" value={recipe.cuisine} />
            <DetailItem label="Fabrication Method" value={recipe.cookingMethod} />
            <DetailItem label="Optimal Consumption Window" value={recipe.mealType} />
            <DetailItem label="Est. Fabrication Time" value={recipe.cookingTime} />
          </div>
        </section>
        
        {recipe.cookingInstructions && recipe.cookingInstructions.length > 0 && (
          <section className="mb-6">
            <h2 className="text-xl md:text-2xl font-semibold mb-4 text-pink-400 uppercase tracking-wider">// Fabrication_Protocol</h2>
            <ol className="list-none space-y-3">
              {recipe.cookingInstructions.map((step: string, index: number) => (
                <li key={index} className="text-neutral-300 leading-relaxed flex">
                  <span className="text-cyan-400 font-bold mr-3 select-none">{`${index + 1}. `}</span>
                  <span>{step}</span>
                </li>
              ))}
            </ol>
